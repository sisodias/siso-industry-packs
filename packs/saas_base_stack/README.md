# SaaS base stack (horizontal template layer, not a vertical)

`saas_base_stack` · **medium** priority · horizontal · status `queued`

**Why:** Shaan: SaaS is not an industry — everything comes under it. What he MEANT is the BASE STACK every SaaS/product needs: landing page, blog, auth, ecommerce, Stripe/payment collection. Reframed as 'saas_base_stack' — a horizontal template layer, not a vertical.


## Run the research

Prompt: **https://siso-superapp-forge.pages.dev** → find `saas_base_stack` → Copy research prompt.
