# Cannabis dispensaries and delivery

`cannabis_dispensary` · **medium** priority · business · status `queued`

**Client:** LC

**Why:** Real shipped app exists (DISPO, Thailand). The Forge run here is about widening from one app to the industry's full software stack. | Client: LC. DISPO already shipped for this vertical.

**Existing SISO asset:** `SISO_Agency/apps/dispo (245M) — Next.js 16, Drizzle+Neon, Clerk, Omise (PromptPay/LINE Pay), Loyverse POS sync, LINE SDKs, OpenRouter+Replicate`

**Verified state:** READ 2026-09-13: genuine operating app with real POS sync and payments — the strongest existing base of the set. Extend, do not restart.


## Run the research

Prompt: **https://siso-superapp-forge.pages.dev** → find `cannabis_dispensary` → Copy research prompt.
