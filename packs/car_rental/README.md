# Car rental operators (independent / small fleet)

`car_rental` · **high** priority · business · status `queued`

**Client:** existing clients + possible NM lead

**Why:** Shaan named as a target vertical. Adjacent base exists (bike-rental, five-star-hire) so the booking/fleet spine is partly precedented. | Multiple existing clients have car rental businesses; strong client demand. Possible lead via NM.

**Existing SISO asset:** `SISO_Agency/clients/bike-rental (597M, delivered 2025-11, remote bike-rental-template.git); clients/five-star-hire (4.7M, 2025-03)`

**Verified state:** UNVERIFIED — adjacent vertical (bike hire), not car rental. Read before assuming reuse.


## Run the research

Prompt: **https://siso-superapp-forge.pages.dev** → find `car_rental` → Copy research prompt.
